          ...
          {TABS.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex flex-col items-center justify-center py-2 gap-0.5 transition-colors ${active ? 'text-blue-500' : 'text-gray-400'}`}>
                <Icon active={active} />
                <span className={`text-xs font-medium ${active ? 'text-blue-500' : 'text-gray-400'}`}>{t.label}</span>
              </button>
            )
          })}
        </div>
