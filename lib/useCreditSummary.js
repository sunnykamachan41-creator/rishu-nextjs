...
export function useCreditSummary({
  ...
  syncKey           = 0,
  exemptions        = [],   // 互換性のため残す
  recognizedCourses = [],   // recognized_courses テーブルのデータ（単位認定）
}) {
  return useMemo(() => {
    ...
    // ── Step 2.5: 単位認定（recognized_courses）を加算 ───────────────────────────
    // enrollment に存在しない course_id のみ追加（二重計上防止）
    if (recognizedCourses?.length) {
      // course_id ベースのコースマップ（カタログから検索用）
      const courseMapById = new Map()
      for (const c of courses) {
        if (c.course_id && !courseMapById.has(c.course_id)) courseMapById.set(c.course_id, c)
        if (c.class_id  && !courseMapById.has(c.class_id))  courseMapById.set(c.class_id, c)
      }
      const alreadyCounted = new Set(completedCourses.map(c => c.courseId))
      for (const rc of recognizedCourses) {
        const cid = rc.course_id
        if (!cid || alreadyCounted.has(cid)) continue
        const course = courseMapById.get(cid)
        if (!course) continue
        alreadyCounted.add(cid)
        completedCourses.push({
          courseId: cid,
          classId:  '',
          ...
          tags:     course.tags
            ? String(course.tags).split('|').map(t => t.trim()).filter(Boolean)
            : [],
          grade:    null,
          semester: null,
        })
    ...
    // ── Note: 単位認定（Exemption）は enrollment! 経由で集計 ──────────────────────
    // recognized=TRUE の授業は enrollment! に登録済みのため、
    // selectedIds / completedIds に含まれ Step 2-5 で自動的にカウントされる。
    // 旧 Step 6 (localStorage exemptions 直加算) は削除済み（二重計上防止）。
  ...
  // syncKey を依存に含め、timetable エントリ変更時に強制再計算する
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [courses, selectedIds, enrollmentYear, maxGrade, syncKey, recognizedCourses])
}
