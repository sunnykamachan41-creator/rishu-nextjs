 ...
 */
export function computeGraduationResults(studentRows, ruleRows, categoryFormulaRows = []) {
  // ── Parse & partition rules (done once) ────────────────────────────────────
  const rules = ruleRows.map(parseRule).filter(Boolean)

  const globalRules  = rules.filter(r => r.rule_type === 'GLOBAL')
  const classRules   = rules.filter(r => r.rule_type === 'CLASS')
  const specialRules = rules.filter(r => r.rule_type === 'SPECIAL')
  ...
  for (let idx = 0; idx < validStudents.length; idx++) {
    ...
    // Evaluate each group
    const globalEvals  = globalRules.map(r => ({ ...evalRule(r, creditMap), ruleType: 'GLOBAL' }))
