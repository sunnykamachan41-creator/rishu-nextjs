 ...
 */
export async function upsertEnrollment({ classId, courseId, year, semester, status, academic_year, studentId = STUDENT_ID() }) {
  const { _enrollmentRows } = await fetchAllSheets(studentId)

  // 新スキーマのヘッダー定義（academic_year を追加）
  const NEW_SCHEMA = ['student_id', 'class_id', 'course_id', 'status', 'year', 'semester', 'academic_year']
