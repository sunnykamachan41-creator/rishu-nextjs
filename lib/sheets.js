...
export async function upsertEnrollment({ classId, courseId, year, semester, status, academic_year, is_temporary = false, memo = null, studentId = STUDENT_ID() }) {
  ...
  if (rowIndex !== -1) {
    ...
    // AUDIT は COMPLETED を上書きしない
    // （単位認定済みの授業を聴講登録しても取得済み状態を維持する）
    const existingStatus = rows[rowIndex][statusCol]
    if (!(status === 'AUDIT' && existingStatus === 'COMPLETED')) {
...
/**
 * users シートの同意フラグ列を更新する。
 *
 * 列が存在しない場合はヘッダー行を自動拡張して追加する。
 * 対象列: terms_accepted | terms_accepted_at | privacy_accepted | privacy_accepted_at
 */
export async function updateUserConsent(studentId, {
  terms_accepted    = false,
  privacy_accepted  = false,
} = {}) {
  const normalizedSid = normalizeId(studentId)
  if (!normalizedSid) throw new Error('[updateUserConsent] student_id is empty')

  const sheets = getSheetsClient()
  const rows   = await getRange('users')
  if (!rows || rows.length === 0) throw new Error('[updateUserConsent] users sheet is empty')

  const headers = [...(rows[0] ?? [])]

  // 列インデックスを確認し、なければヘッダーを拡張する
  const CONSENT_COLS = ['terms_accepted', 'terms_accepted_at', 'privacy_accepted', 'privacy_accepted_at']
  let extended = false
  for (const col of CONSENT_COLS) {
    const idx = headers.findIndex(h => normalizeId(String(h)) === col)
    if (idx === -1) {
      headers.push(col)
      extended = true
    }
  ...
  if (extended) {
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId:    SPREADSHEET_ID(),
      range:            `users!A1:${colToLetter(headers.length - 1)}1`,
      valueInputOption: 'RAW',
      requestBody:      { values: [headers] },
    })
    console.log('[updateUserConsent] extended users headers:', headers)
  }

  const colIdx = (name) => headers.findIndex(h => normalizeId(String(h)) === name)
  const sidCol         = colIdx('student_id')
  const termsCol       = colIdx('terms_accepted')
  const termsAtCol     = colIdx('terms_accepted_at')
  const privacyCol     = colIdx('privacy_accepted')
  const privacyAtCol   = colIdx('privacy_accepted_at')

  const dataRows   = rows.slice(1)
  const matchIdx   = dataRows.findIndex(
    r => normalizeId(String(r[sidCol] ?? '')) === normalizedSid
  )
  if (matchIdx === -1) throw new Error(`[updateUserConsent] student_id not found: ${normalizedSid}`)

  const sheetRow  = matchIdx + 2  // 1-indexed + header row
  const updatedRow = [...dataRows[matchIdx]]
  while (updatedRow.length < headers.length) updatedRow.push('')

  const now = new Date().toISOString()
  if (terms_accepted) {
    updatedRow[termsCol]   = 'true'
    updatedRow[termsAtCol] = now
  }
  if (privacy_accepted) {
    updatedRow[privacyCol]   = 'true'
    updatedRow[privacyAtCol] = now
  }
  ...
  invalidateCache(studentId)
  console.log('[updateUserConsent] updated consent:', { studentId: normalizedSid, terms_accepted, privacy_accepted })
}
...
export async function updateProgressAuto(studentId = STUDENT_ID()) {
  ...
  // Route B の二重計上防止に使う「COMPLETED 済み course_id」セット
  // AUDIT / FAILED / IN_PROGRESS は単位に含まれないため、認定科目のスキップ対象外とする
  const completedEnrolledCourseIds = new Set(
    normalizedEnrollment
      .filter(e => e.status === 'COMPLETED')
      .map(e => e.course_id)
  )
  // selectedIds 系の処理には全ステータスを含む元セットも必要
  ...
  const recognizedProgressRows = studentRecognized
    .filter(r => {
      ...
      // COMPLETED の enrollment が同一 course_id にある場合のみスキップ（二重計上防止）
      // AUDIT / FAILED / IN_PROGRESS は単位に含まれないため、認定クレジットをスキップしない
      return !completedEnrolledCourseIds.has(cid)
    })
    .map(r => {
...
/**
 * 全登録ユーザーに通知を一括送信する（管理者専用）。
 * users シートから全 student_id を取得し、notifications シートに一括 append する。
 *
 * @returns {Promise<number>} 送信したユーザー数
 */
export async function createBroadcastNotification({ title, message, link = '', type = 'broadcast' }) {
  // 全ユーザー取得
  const userRows = await getRange('users').catch(() => [])
  if (!userRows || userRows.length < 2) return 0

  const headers = userRows[0] ?? []
  const sidCol  = headers.findIndex(h => normalizeId(String(h)) === 'student_id')
  if (sidCol === -1) return 0

  const userIds = userRows.slice(1)
    .map(r => String(r[sidCol] ?? '').trim())
    .filter(Boolean)
  ...
  // Sheets API に一括 append（1 API コールで完結）
  const sheets = getSheetsClient()
  await sheets.spreadsheets.values.append({
    spreadsheetId:    SPREADSHEET_ID(),
    range:            `${NOTIF_SHEET}!A1`,
    valueInputOption: 'RAW',
    insertDataOption: 'INSERT_ROWS',
    requestBody:      { values: newRows },
  })

  console.log(`[createBroadcastNotification] sent to ${userIds.length} users: "${title}"`)
