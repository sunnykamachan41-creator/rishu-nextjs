...
if (summaryRows.length >= 2) {
  ...
  entry.data = {
    ...
    userDepartment,            
    users: rowsToObjects(userRows),                           // current user's department_id from users sheet
...
export async function upsertEnrollment({ classId, courseId, year, semester, status, studentId = STUDENT_ID() }) {
  ...
  // 新スキーマのヘッダー定義
  const NEW_SCHEMA = ['student_id', 'class_id', 'course_id', 'status', 'year', 'semester']
...
export async function upsertUserDepartment(departmentId, studentId = STUDENT_ID()) {
  ...
  // rawRows[i] に対応するシート行番号 = rawRowNums[i]（1始まり、ヘッダー込み）
  const rawRows    = rows.slice(1)
  const rawRowNums = rawRows.map((_, i) => i + 2)  // 1-based + header offset

  // この student_id に一致する行をすべて探す（重複検出のため全件走査）
  ...
  if (matchIndices.length === 0) {
    ...
    console.log('[upsertUserDepartment] appended new row:', { studentId: normalizedSid, departmentId: normalizedDept })
    ...
    console.log('[upsertUserDepartment] updated row:', { studentId: normalizedSid, departmentId: normalizedDept, sheetRow: keepRow })
...
async function _doBootstrapUser(normalizedEmail) {
  try {
    ...
    if (emailIdx === -1 || studentIdIdx === -1 || deptIdx === -1) {
      ...
      console.log('[bootstrapUserIfNeeded] initialized users sheet:', { email: normalizedEmail, student_id: firstStudentId })
    ...
    // appendRow の直前にシートを再取得して email が存在しないことを確認する。
    const freshRows     = await getRange('users').catch(() => [])
